...
async function popularBanco() {
  ...
  try {
    console.log('\n╔════════════════════════════════════════════╗');
    console.log('║   SISTEMA DE BUSCA - DATABASE SEEDER      ║');
    console.log('╚════════════════════════════════════════════╝\n');

    console.log('🗑️  Limpando tabela de registros...');
    await client.query('TRUNCATE TABLE registros RESTART IDENTITY');

    const totalRegistros = 5000; // Mínimo de 5.000 registros conforme demanda
    ...
    for (let i = 1; i <= totalRegistros; i++) {
      ...
      if (i % 500 === 0) {
        console.log(`✅ ${i} registros criados...`);
      }
    }
